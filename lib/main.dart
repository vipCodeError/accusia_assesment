    ...
    builder: (context, snapshot) {
      ...
      for (int i = 0; i < eProduct.categories.length; i++) {
        Categories categories = Categories(
            id: eProduct.categories[i].id, name: eProduct.categories[i].name);
        DBProvider.db.createEmployee(categories);
        // Products
        for (int j = 0; j < eProduct.categories[i].products.length; j++) {
          Products products = Products(
              id: eProduct.categories[i].products[j].id,
              name: eProduct.categories[i].products[j].name,
              cat_Id: eProduct.categories[i].id);
          DBProvider.db.createProduct(products);
          //variants
          for (int k = 0;
              k < eProduct.categories[i].products[j].variants.length;
              k++) {
            Variants variants = Variants(
                color: eProduct.categories[i].products[j].variants[k].color,
          ...
          //tax
          Tax tax = Tax(
              name: eProduct.categories[i].products[j].tax.name,
        ...
        for (int m = 0;
            m < eProduct.categories[i].child_categories.length;
            m++) {
          ChildCat childCat = ChildCat(
              child_cat: eProduct.categories[i].child_categories[m],
              cat_id: eProduct.categories[i].id);
        ...
        for (int p = 0; p < eProduct.rankings[n].products.length; p++) {
          if (n == 0) {
            ViewCount viewCount = ViewCount(
                id: eProduct.rankings[n].products[p].id,
                view_c: eProduct.rankings[n].products[p].viewCount);
            DBProvider.db.createViewCount(viewCount);
          } else if (n == 1) {
            OrderCount orderCount = OrderCount(
                id: eProduct.rankings[n].products[p].id,
                order_c: eProduct.rankings[n].products[p].orderCount);
            DBProvider.db.createOrderCount(orderCount);
          } else if (n == 2) {
            ShareCount shareCount = ShareCount(
                id: eProduct.rankings[n].products[p].id,
                share_c: eProduct.rankings[n].products[p].mostShares);
